...
const createDocumentResolver = (documentType?: DocumentType): ResolveFn<DocumentAndAuthor> => {
    return (route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<DocumentAndAuthor> => {
        ...
        let nbr = route.queryParamMap.get('nbr');
        if (documentType == null) {
            documentType = route.queryParamMap.get('type') as DocumentType;
        }

        return inject(DocumentService).getDocument(documentType = documentType, tags = tags, search = search, uuid = uuid, slug = slug, years = years, sort = sort, authors = authors);
    };
};
